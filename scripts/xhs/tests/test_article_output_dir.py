...
def test_resolve_output_dir_uses_pinyin_name(tmp_path: Path) -> None:
    ...
    source = tmp_path / "content/docs/2026/06/reading-category__post-13f67e2873.md"
...
def test_resolve_output_dir_reuses_same_source(tmp_path: Path) -> None:
    ...
    (dir_path / "manifest.json").write_text(
        json.dumps({"source": "content/docs/2026/06/reading-category__post-13f67e2873.md"}),
        encoding="utf-8",
    )
...
def test_resolve_output_dir_disambiguates_collision(tmp_path: Path) -> None:
    ...
    dir_path = articles_root / "te-si-la-yu-wai-xing-ren"
    dir_path.mkdir(parents=True)
    other_source = tmp_path / "content/docs/2026/06/reading-category__post-9999999999.md"
    (dir_path / "manifest.json").write_text(
        json.dumps({"source": "content/docs/2026/06/reading-category__post-9999999999.md"}),
        encoding="utf-8",
    )

    name = resolve_article_output_dir(
        "特斯拉与外星人",
        tmp_path / "content/docs/2026/06/reading-category__post-13f67e2873.md",
        articles_root=articles_root,
        repo_root=tmp_path,
    )
